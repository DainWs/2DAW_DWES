<div class="widget">
    ...
    <form action="<?= $_SERVER['APP_BASE_URL'] . '/SessionController/doSigningPost'; ?>" enctype="application/x-www-form-urlencoded" method="POST" autocomplete="off">
        ...
        <?php if(isset($DATA[ERRORS][CONTROLLER_SESSION_SINGING]['name'])): ?>
            <p class="error"><?= $DATA[ERRORS][CONTROLLER_SESSION_SINGING]['name'] ?? ''; ?></p>
        <?php endif; ?>
        ...
        <?php if(isset($DATA[ERRORS][CONTROLLER_SESSION_SINGING]['password'])): ?>
            <p class="error"><?= $DATA[ERRORS][CONTROLLER_SESSION_SINGING]['password'] ?? ''; ?></p>
        <?php endif; ?>
